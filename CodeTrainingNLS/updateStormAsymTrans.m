function Storm = updateStormAsymTrans(Storm)
    
    residuals = []; N = 0; velAsym = [];
...
%             Storm(i).B, Storm(i).Rn, Storm(i).Xn, Rcut(i));
        [Y, ~, idx] = VHolCNLS_Iter(Storm(i).AsymR, Storm(i).Vm, Storm(i).Rmax, ...
            Storm(i).B, Storm(i).Rn, Storm(i).Xn, Inf);
        dir = dir(idx);
...
%         velAsymIter = sqrt(uAsym.^2 + vAsym.^2);
%         velAsym = [velAsym; velAsymIter];
        Storm(i).velAsym = sqrt(uAsym.^2 + vAsym.^2);
